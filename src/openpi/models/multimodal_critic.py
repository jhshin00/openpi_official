...
#         # Note: Critic needs to handle padding differently from Actor
#         # - Actor (Pi0): Padding = "repeat last action" (valid for generation)
#         # - Critic: Padding = "no action" (should be ignored for value estimation)
#         # Apply padding mask before embedding to avoid unnecessary computation
#         if actions_is_pad is not None:
#             # Zero out padded actions before embedding
#             masked_actions = actions * (1 - actions_is_pad.astype(jnp.float32)[..., None])
#         else:
#             masked_actions = actions
            
#         act_tokens = self.action_embed(masked_actions.reshape(-1, self.action_dim))  # (B*H, D)
...
#         if actions_is_pad is not None:
#             # Zero out padded action tokens (additional safety)
#             act_tokens = act_tokens * (1 - actions_is_pad.astype(jnp.float32)[..., None])
#             # Create attention mask for action sequence
#             # True for valid tokens, False for padded tokens
#             act_attn_mask = ~actions_is_pad  # (B, H)
#             act_attn_mask = act_attn_mask[:, None, :, None]  # (B, 1, H, 1)
#             K_len = self3_out.shape[1]
#             act_attn_mask = jnp.broadcast_to(act_attn_mask, (B, 1, actions.shape[1], K_len))
#         else:
#             act_attn_mask = None

#         cross2_out = self.cross_attn_out(act_tokens, self3_out, mask=act_attn_mask, deterministic=not train)

#         # Better pooling strategy for variable-length sequences
#         if actions_is_pad is not None:
#             # Weighted average based on valid tokens
#             valid_mask = (1 - actions_is_pad.astype(jnp.float32))[:, :, None]  # (B, H, 1)
#             weighted_sum = jnp.sum(cross2_out * valid_mask, axis=1)  # (B, D)
#             valid_count = jnp.sum(valid_mask, axis=1)  # (B, 1)
#             pooled = weighted_sum / (valid_count + 1e-8)  # Avoid division by zero
#         else:
#             # Simple mean pooling for fixed-length sequences
#             pooled = cross2_out.mean(axis=1)
            
#         x = self.q_head_ln(pooled)
#         q = self.q_head_mlp(x).squeeze(-1)
#         return q
